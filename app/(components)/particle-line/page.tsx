// pages/particleLine.tsx
"use client";
import { useState } from 'react';
import { ParticleLineTrailCursor } from 'funky-cursor'; // Ensure this is the correct import path
import Layout from '@/components/Layout';

const ParticleLineTrailPage = () => {
  const [color, setColor] = useState('#cc00ff'); // Default color
  ...
  return (
    <Layout>
      <div className="p-4 md:p-8">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-4 text-center">Particle Line Trail Cursor</h1>
        <p className="mb-4 text-sm sm:text-base md:text-lg">
          The ParticleLineTrailCursor adds a particle line effect to the cursor. You can customize the properties below:
        </p>

        {/* Code Example Section */}
        <pre className="bg-gray-800 p-3 sm:p-4 md:p-6 rounded-md text-white text-xs sm:text-sm md:text-base overflow-auto mb-4">
          {`import { ParticleLineTrailCursor } from 'funky-cursor';
...
`}
        ...
        <div className="mt-8 border-2 border-gray-700 p-3 sm:p-4 md:p-6 relative">
          <ParticleLineTrailCursor 
            ...
            lifespan={lifespan} 
          />
          <p className="text-sm sm:text-base md:text-lg">Move your cursor around to see the particle line trail effect!</p>
        </div>
      </div>
    </Layout>
  );
...
export default ParticleLineTrailPage;
