...
import { LaserTrailCursor } from 'funky-cursor'; // Ensure this is the correct import path
import Layout from '@/components/Layout';

const LaserTrailPage = () => {
  const [color, setColor] = useState('#00ffff'); // Default color
  ...
  return (
    <Layout>
      <div className="p-4 md:p-8">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-4 text-center">Laser Trail Cursor</h1>
        <p className="mb-4 text-sm sm:text-base md:text-lg">
          The LaserTrailCursor creates a laser-like trail effect following your cursor. Customize the properties below:
        </p>

        {/* Code Example Section */}
        <pre className="bg-gray-800 p-3 sm:p-4 md:p-6 rounded-md text-white text-xs sm:text-sm md:text-base overflow-auto mb-4">
          {`import { LaserTrailCursor } from 'funky-cursor';
...
`}
        ...
        <div className="mt-8 border-2 border-gray-700 p-3 sm:p-4 md:p-6 relative">
          <LaserTrailCursor 
            ...
            speed={speed} 
          />
          <p className="text-sm sm:text-base md:text-lg">Move your cursor around to see the laser trail effect!</p>
        </div>
      </div>
    </Layout>
  );
